...
class AudioBuffer():
	def __init__(self, speaker, sink):
		self.buffer = np.zeros(shape=(sink.NUM_SAMPLES * sink.BUFFER_FRAME_COUNT, sink.NUM_CHANNELS), dtype='int16')
		self.buffer_pointer = 0
		self.speaker = speaker

	def fill_buffer(self, frame, NUM_SAMPLES):
		self.buffer[(self.buffer_pointer * NUM_SAMPLES) : ((1 + self.buffer_pointer) * NUM_SAMPLES)] = frame
...
class BufferAudioSink(voice_recv.BasicSink):
	...
	def _build_frame(self, voice_data):
		try:
			frame = np.ndarray(shape=(self.NUM_SAMPLES, self.NUM_CHANNELS), dtype='int16', buffer=voice_data.pcm)
...
class AudioListener:
	...
	def _transcribe(self, speaker, pcm_s16le):
		...
		if self.audio_sink.caller_id or any(word in message for word in self.activate_words):
			self.audio_sink.caller_id = speaker
			self.messages.append((speaker, message))

	async def _send_message_to_channel(self, member, message): # future transcriber feature
		speaker_nick = str(member.nick if member.nick is not None else member) if member is not None else 'N/A'
	...
	def _get_context(self, message):
		if self.chat_history:
			...
			context = [{"role": "system", "content": self.vc_system_prompt},
